...
- **Combine with Attestation**: Back local checks with server-verified device attestation (see @MASWE-0055) so the backend can act on integrity signals even if local checks are bypassed.
